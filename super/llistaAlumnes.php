...
<HTML>
	<HEAD>
		<meta charset='utf-8'/>
		<title> Alumnes del Centre - INS Esteve Terradas i Illa </title>
		<?php menu_consultes($usuari); ?>
	</HEAD>
	
	<BODY>
		<h1> Alumnes de l'Institut - INS Esteve Terradas i Illa </h1>
			<form name='formulariConsultaAlumnes' id='formulari' action='llistaAlumnes.php' method='post' onsubmit=''>
			...
			if(isset($_POST["tancaSessio"])){			
				...
				if($triaTipus != false){
					...
					if($triaTipus == 1 || $triaTipus == 4){
						$arrayDades = obtenirAlumnesCentre($triaTipus, false);
					}else if($triaTipus == 2){
						$arrayDades = obtenirAlumnesCentre($triaTipus, $gremi);
					}else if($triaTipus == 3){
						$arrayDades = obtenirAlumnesCentre($triaTipus, $cicle);
					}else{
						$arrayDades = false;
					}					

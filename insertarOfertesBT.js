function valida(){
	...
	var requisits = document.formulariPeticioFCT.requisits.value;
	...
	if( requisits == null || requisits.length == 0 || /^\s+$/.test(requisits) ) {
		missatge = missatge+'- El camp dels requisits a realitzar per l\'aspirant no pot estar buit, és obligatori! \n \n';
		validacio = false;
	}else{
		if(requisits.length > 900){
			missatge = missatge+'- El camp dels requisits a realitzar per l\'aspirant és massa gran, ha de contenir un màxim de 900 caràcters! \n \n';
			validacio = false;
		}
	}
	...
	if(tasques.length > 0){
		if(tasques.length > 900){
			missatge = missatge+'- El camp de les tasques a realitzar per l\'aspirant és massa gran, ha de contenir un màxim de 900 caràcters! \n \n';
			validacio = false;
		}
